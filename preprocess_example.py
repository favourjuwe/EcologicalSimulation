...
ROOT_DIR = 'data/'
IN1 = ROOT_DIR + \
    "A25A.TA..BHZ.2011.025.00.00.00.000-\
    2011.026.00.00.39.000.rm.scale-AUTO.SAC"
IN2 = ROOT_DIR + \
    "BMN.LB..BHZ.2011.025.00.00.00.023\
    -2011.026.00.00.38.998.rm.scale-AUTO.SAC"
...
class StreamToFile(ConsumerPE):
    ...
    def _process(self, data):
        ...
        try:
            ...
            # maybe there's no "%s" in the string so we ignore count
            fout = self.file_dest
...
def filter(str1, freqmin=0.01, freqmax=1., corners=4, zerophase=False):
    str1.filter('bandpass', freqmin=freqmin, freqmax=freqmax,
...
class PreTaskPE(IterativePE):
    ...
    def _process(self, data):
        ...
        rf = data[2]
        # this is an odd one
        # - need to put one of the tuple values into the parameters list
...
streamToFileWhitened = StreamToFile(
    ROOT_DIR + 'OUTPUT/WHITE/%s_preprocessed.SAC')
...
graph.connect(whiten, IterativePE.OUTPUT_NAME,
              streamToFileWhitened, StreamToFile.INPUT_NAME)
